class ObservatoriosController < ApplicationController
  # skip_before_filter :define_cidade_corrente
  before_filter :login_required, :except => [ :index ]
  before_filter :obter_observatorio, :except => [ :new, :create ]
  before_filter :obter_contadores, :only => [ :index, :comentarios, :apoios ]

  def index
    # @topicos = Topico.scoped(@observatorio.contexto).paginate(:all, :per_page => 5, :page => params[:page], :order => "topicos.id DESC") if @observatorio
    @topicos = Topico.com_tags(@observatorio.tag_ids).nos_locais(@observatorio.locais).paginate(:all, :per_page => 50, :page => params[:page], :order => "topicos.id DESC") if @observatorio
  ...
  def comentarios
    @comentarios = Comment.paginate(:all,
  ...
  def obter_observatorio
    @observatorio = logged_in? ? current_user.observatorios.first : nil
