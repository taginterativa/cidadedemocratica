class Admin::TagsController < Admin::AdminController
  ...
  protect_from_forgery :except => [:livesearch]
  # Oferece a busca inteligente 
  def livesearch
    if params[:tag]
      word = params[:tag][:de] || params[:tag][:para]
      auto_complete_responder_for_tags word.strip
    end
  end
  ...
  def auto_complete_responder_for_tags(value)
    tags = Tag.find(:all,
                    ...
                    :limit => 8)
    if tags
      render :partial => 'form_livesearch',
             :locals => { 
               :tags => tags,
               :typed => value
             }
  ...
  def transfere_observatorios(tag_de, tag_para)
    ...
    Observatorio.com_tag(tag_de.name).each do |o|
      o.tags.delete(tag_de)
      o.tags << tag_para unless o.tag_ids.include?(tag_para.id)
    end
  end
end