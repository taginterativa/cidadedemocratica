ActiveSupport::Inflector.inflections do |inflect|
  ...
  inflect.plural(/^(cidad|irm|m)ão$/i,  '\1ãos')
  inflect.plural(/^(alem|c|p)ão$/i,  '\1ães')

  # Sem acentos...
  inflect.plural(/ao$/i,  'oes')
  inflect.plural(/^(cidad|irm|m)ao$/i,  '\1aos')
  ...
  nomes_compostos = {
    "poder_publico" => "poderes_publicos",
    "gestor_publico" => "gestores_publicos"
  }
  nomes_compostos.each do |key, value|
    inflect.plural(/^#{key.to_s}$/, value)
    inflect.singular(/^#{value}$/, key.to_s)
  end

  irregulares = {
    'pais' => 'paises', # Plural de país (=nação), não pais (=companheiro da mãe)
    'país' => 'países',
    'user' => 'users',
    'session' => 'sessions',
    'historico_de_login' => 'historico_de_logins'
  }
