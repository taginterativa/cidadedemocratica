...
/* Load XML Object
   (Parameters) rssFeed:'RSS File' NumItens:'Numero de itens a mostrar' */

function ReadRSS(rssFeed, numItens) 
{
	//rssTitle = document.getElementById(Title);	
	//rssBody = document.getElementById(Body);

	rssTitle = document.getElementById('widgetCidade_titulo');	
	rssBody = document.getElementById('widgetCidade_corpo');
...
function SetRSSTemplates(totalItens)
{
	...
	if (rssBody)
	{
		...
		for(var i=0; i< (items.length >= totalItens ? totalItens : items.length); i++) 
		{
